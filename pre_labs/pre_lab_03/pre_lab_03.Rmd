...
**Answer** The first line of the red warning message says "one or more parsing issues," and to put 'problems()' into the code. It uses the first ten rows to set the data type, including row and column as <int> fields, and expected and actual, which is the postal codes column, as <chr> fields. 
...
**Answer** There are now no parsing errors. 
...
**Answer** The `Vendor Name` field is a character, <chr>. The "Amount" field is a <dbl> or double, and the "Date" column is a <chr> or character field. 
...
**Answer** 547 zip codes do not have five characters. The zeroes that start some of these zip codes are missing, especially if the Excel spreadsheet of the data was saved prior to us opening the data in R. 
...
**Answer** In the category row, there are other values versus "Grant" and "Loan," including "N/A," "L," and "Contract."
...
**Answer** There are two outliers first, with amounts up to 60 and 95.98 dollars, which are a lot smaller amounts than the rest of the grants and loans. 
...
**Answer** The header names are now all lowercase and spaces were replaced with underscores. 
...
**Answer** 50 rows might be duplicates.
...
**Answer** The new dataframe now has 17740 rows. 
...
**Answer** The zip codes under zip5 column all now have five digits instead of nine digits. It's important to note some have letters and/or four digits. 
...
**Answer** The zip code that has gotten the most amount of money from the Maryland Tourism Board is 20001, or Washington D.C. I think this makes sense as D.C. is within the Maryland area and it a huge tourist city. 

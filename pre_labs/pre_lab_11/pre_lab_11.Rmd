...
**Answer** There are only two columns and come from the 1st dataframe. The first column is the date and the second is the news release/news item column. 
...
**Answer** Other information that would be helpful would be links/URLs to the press releases and if we had those, we could scrape the text of the press releases and place them into the data frame too. This could become a searchable database.
...
**Answer** The output suggests there are 122 <ul> tags in the HTML source. It looks like the ones that start listing items start at item 17. The first 16 are social nmedia links and menu items versus listing items, so technically 105 listing items if you remove the menu items and social media links. 
...
**Answer** The first 16 links show social media links and other tabs for menu items, and the press releases (listing items) start at item 17. 
...
**Answer** There is only one output that includes all the information in one block, such as the date, title and more. It also includes the ones that don't have listing-item class. 

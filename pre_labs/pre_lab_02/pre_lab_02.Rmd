...
**Answer** It's important to make a new variable so we can keep track of the certain data we filtered. Also, it could avoid us from overwriting the original data set we were given. 
...
**Answer:** Instead of <chr> underneath the expenditure_date title, it now says <date>. This puts the dates in actual date form (mdy) in R. 
...
**Answer** In the new 'month' column, it rounded all of these days in the month to the first of each month in order to help R calculate the total amount of expenditures each month. (June 29 would now be June 1).
...
**Answer** The code is adding all of the votes and taking the total amount of votes that Moore received and dividing this by the total amount of votes of all candidates to make a percentage (in decimal form) of total votes for Moore.
...
**Answer** The data now shows the highest percentage of votes Moore received first and goes down from there. It changed this way because we added arrange in descending order into the code. 
...
**Answer** It isn't necessarily more interesting or useful due to the fact that these counties being shown in ascending order have a smaller amount of people living there. They are the smallest counties in Maryland. However, Moore could use this to visit counties where he doesn't have much support to try to gain that support or more positive feelings toward him. 
...
**Answer** One similar function is str_to_lower, which converts to lower case, and the other is str_to_title, which converts to title case where the first letter of each word is capitalized. I found this on the website stringr. 
...
**Answer** The mutate statement is finding when the word 'Maryland,' or another state listed below is found with a space before and after the name in the address column, and changing it to 'MD' or the other state abbreviations in the new state column. 
...
**Answer** Yes, this answer makes sense to me. It is showing the total amount of expenses from each state.
...
1. How many journalism courses have the word 'broadcast' in their titles?

2. What percentage of votes did Cox receive in each Maryland county?

3. How many courses in the Biochemistry department have at least 80 seats?

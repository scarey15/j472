...
**Answer** The dataframe it outputs includes states, 2016 votes of states for Trump or Clinton, the percent Trump won, percentage of small businesses who received the PPE loan and the amount per employee (per capita). 
```{r}
...
**Answer** The scatterplot shows a general trend that as the percentage of votes for Trump increases, the percentage of businesses receiving PPE loans increases as well. It shows there is a relationship but we can still interpret it more closely. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** The scatter plot shows a weaker trend line, with one large outlier to the left. There are a lot of outliers and scatter than the other plot, not really showing a strong increase or decrease trend. 
```{r}
...
**Answer** This outout shows the correlation coefficient for each piece of data. The percentage of businesses receiving the PPE loan have a 0.5 coefficient and per employee has 0.2. When comparing these values on the scale of -1 to 1, the businesses receiving the loans has a more stronger/positive relationship, whereas the per employee coefficient shows a weak relationship. 
...
**Answer** This allows us to see the correlation coefficient, intervals and other values, including the p-value. The p-value is small (0.00008607). This means that there is stronger confidence of a relationship and statistical significance we could study for a story. It probably didn't happen by chance. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** This output shows the coefficient and the p-value of 0.1189. This shows a bigger p-value, meaning that there isn't much of a sign of a strong relationship or statistical significance between the two. It could've happened by chance. 
```{r}
...
**Answer** This table shows the average amount per employee people received between Trump states and Clinton states. It also shows the average for the businesses. This differences between Trump states and Clinton states don't seem too far apart. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The output is somewhat similar to the tests before. It lists out averages, p-value and t-value. The p-value is there at 0.2295. This is not a pattern with much importance or use for a story as the p-value is bigger. There doesn't seem to be much statistical significance and could've of happened by chance. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** This output calculates the averages, p-value and t-values again. This p-value is small at 0.002643, which would mean that this relationship could be used for a story and has statistical significance probably not by chance. 
```{r}

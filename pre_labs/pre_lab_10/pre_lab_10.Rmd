...
**Answer** The columns include the shape area and shape length along with geometry in the data frame. It seems that it is setting up shapes and how long and how big they are to create the map.
...
**Answer** It looks set up like a normal data frame. It shows zip codes and zip code names, like cities (Cumberland for example). It also lists out geometry and shape area and length. These must help create the map we are trying to make. 
...
**Answer** The output is actually the state of Maryland. It divided up all of the cities found within the state with boundary lines. It also shows the latitude and longitude of the state and where each piece of the state is as well. 
...
**Answer** The output includes a list of every county in the country along with a geometry column. It also seems to count states and counties by numbers. It provides zip code numbers and names of each county as well. 
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This output lists all Maryland counties with zip codes, names, their FP number, some other number locations and a geometry column. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This output shows the state divided by county lines, but the bay is missing from the middle of the state. The lines are all filled in. You also can't see if the Potomac line is included or not. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This output not only show the county boundary lines, it also shows the zip code boundary lines. It also considers water and bay boundaries within the state. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer** This output shows each zip code in Maryland how many notices of foreclosures were sent out in each zip code. It's important to note there are more rows than MD zip codes (547 versus 526 zip codes). One zip code listed is 19973 which isn't in MD, and another is labeled "No Zip Code."
...
**Task** Run the following code. Describe the output in the space below.
**Answer** this output added the foreclosure notices data to the data listing zip codes with geometry data. It joined the zip codes together to match the data frame. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This output maps out every zip code boundary and fills in based on number of foreclosure notices. The more foreclosure notices there are, the lighted the blue is (the magma color theme). It also shows the latitude and longitude of the state. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The output provides two data frames, one showing the population of each county with zip codes, and the other showing the number of foreclosure notices for each county with date and time. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This output added the geometry column to the county population data. It's important to note in the population data frame, it lists each county with ", Maryland" which needs to be removed to be able to match county names in the county notices data frame. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** After looking at both of the data frames, we see differences in county names. In the population data, the counties are labeled with ", Maryland." The counties in the notices data doesn't have this added, so these rows wouldn't be able to be joined together. We have to remove the ", Maryland."
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This ouput removes the ", Maryland" from the county names in the population data while keeping the population number and geometry column. Now it can be joined with the notices data frame. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This output joined the population and notices data frames together. It matched the counties and now list off their population numbers, number of notices, date and time, and the geometry column. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This ouput calculates the rate of foreclosure notices within each county and lists it as a column in the data frame. You cans see the county with the highest rate is Charles County, PG County being second. The lowest rate is shown in Montgomery County. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This output maps the county boundaries on the state of MD and shows the rate of foreclosure notices. The higher the rate, the lighter the blue it is. The lightest is Charles County. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This output changes the color palette of the data from the map above. It gives a bigger range of colors within different rates of foreclosure notices. Charles County pops out as the lightest yellow. It makes it pop. 
...
**Answer** This output changed the scale used to see the foreclosure notice rates of each MD county. It provides a bigger range of colors throughout the state, as more counties can now fit between each range because of the scale change. It still shows Charles County popping out with the yellow. 

...
**Answer** There are four columns and 3,822 rows. 
...
**Answer** The code is making a new data frame organizing calls by month, and label=TRUE gives the abbreviation of each month. It then summarizes total call for each month and lists them from most to least. 
...
**Answer** I see now a blank gray space/canvas with no data. 
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** After running the code, I see a bar chart with the x-axis labeled by month and the y-axis labeled by count of calls. (Weight is also known as the total calls.)
...
**Answer** This code reorders the total calls in ascending order so the month with the lowest amount of calls starts on the left and the most ends up on the right. February has the fewest calls and August has the most calls.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** This code switched the axes of the data on the chart. The x-axis is now the count of total calls and the y-axis is the months organized from the most amount of calls at the top and the least at the bottom. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The code got rid of the light-gray background and it's now white, popping out the bars of data (of calls) more clearly.
...
**Answer** This theme gave a light/steel blue background with white lines under the bars. The font seems to have changed to be a bit bigger. It also doesn't have the best spacing with the titles of each axis and their lists. There also is no title.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** This code added a title, provided clearer titles for each axis and a bit of better spacing. It also cites the source (caption) in the bottom right-hand corner.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** This code shows the total amount of calls on a certain date. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** This code makes a line chart with date (as months) as the x-axis and total calls as the y-axis. The lines are very tight together and seems there's a lot going on. We can see some high and low outliers. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The code has the line graph look the same, but a bunch of week breaks that are completely jumbled together. You can't read it. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The code moved the date week breaks on an angle so they are listed out diagonally. It is a bit better to read but still extremely tight together. The spacing needs to be improved still. 
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 

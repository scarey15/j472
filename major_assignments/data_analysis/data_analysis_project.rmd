...
After finding the correlation for danceability and energy to songs' views and streams, we can see that there is statistical significance and that there is a clear connection/relationship between how danceable or energetic a song is and how popular it is on a platform. The correlation values and p-values are low, not close to 1, signifying this statistical significance. (0.08669851 and p-value < 0.00000000000000022; 0.06591307 and p-value < 0.00000000000000022)
...
With the channel counting dataframes, we can now see any outliers from both the licensed dataframe and the unlicensed dataframe. In this channel dataframe (or artists YouTube channel name) we see that a lot of the licensed and official songs are attached to an official YouTube channel. Some examples being Vevo, Company, Sony Music, Records, etc. The unlicensed and unofficial songs seem to be attached to channels with less official names, for example 'BLESSD EL BENDITO 💙', 'Lyrical Lemonade', '30PRAUM'. 

From this new 'artist' data we are able to see that more official artists have more popular songs. The unoffical channels are succcessful butt now as much as the official ones. 
...
licensed_youtube <- cleaned_spotify_and_youtube %>%
  ...
  group_by(title, views, channel, licensed, official_video) %>%
  summarise(count = n()) %>%
  arrange(desc(views))
...
# Counting the Channels for licensed songs (artists)
licensed_channel_count <- licensed_youtube %>% 
  group_by(channel) %>% 
  summarise(total_count = sum(count)) %>% 
  arrange(desc(total_count))

# Counting the Channels for not-licensed songs (artists)
unlicensed_channel_count <- not_licensed_youtube %>% 
  group_by(channel) %>% 
  summarise(total_count = sum(count)) %>% 
  arrange(desc(total_count))
...
## Narrative for question
We decided to include this question as a replacement for a question dating back to milestone_01. Since we can't compare data from Apple Music to Spotify or YouTube because the dataframe doesn't include it, we will compare Spotify and YouTube. To do this, we summed up the total views YouTube received in this dataframe and how many total streams Spotify has in the dataframe. To then flesh out this data more, we separated Spotify streams and stats and YouTube views and stats to compare how artists perform on both platforms. We created dataframes to show the average streams and average views of the top 100 artists, and then all of the artists in the dataframe. We used Datawrapper to then create a line graph to compare how artists' views on YouTube related to their streams on Spotify. 
...
It's important to note that there are many songs that contain N/A values for streams and views, which is why we added "na.rm = TRUE" to the code below. It could be possible that filling in these N/A values could have shown different numbers for Spotify and YouTube. 

Also, this data doesn’t contain any other form of content on both platforms, Spotify has podcasts, and YouTube has normal videos that aren’t considered music. These are not taken into account here, so there could be a bigger difference between the two, if we had more data to go off of.

After averaging streams and views for all of the artists, we compared their performance on these platforms. We first took a look at the top 100 artists listed by most to least average streams, and placed their view counts on this line graph as well. Seen in the left side of the line graph either through the link below or the screenshot, XXXTENTACION has a high number of average Spotify streams that does not correlate to the amount of average YouTube views. We can see this in numbers from the dataframe with 1,322,435,170 (about 1.3 billion) average streams and 254,637,432 (about 254 million) average views. That is a big difference. Looking at the graph as a whole, it seems that streams and views for many artists are not on similar paths and many have some wide differences.

When comparing all of the artists listed by most to least average streams and how their views correlate, we seem to get the same trend with a few outliers. Average views seem to decrease along with average streams, but from the average view lines, they show waves of changes, most not correlating to the average streams. One example of an outlier is CoComelon, which is the data shown more towards the right of the picture (and second graph linked below) that shows a low average stream count and a huge average view count on YouTube. 

Overall, it seems that averaged out, streams from Spotify and views from YouTube do not match well for many artists, as many have wildly different performance averages for both platforms. This could be due to many just wanting to listen to music if they are driving, doing work, at the gym, hanging out with friends, etc. YouTube could be used more when at home wanting to watch videos, and especially with CoComelon as many parents put these videos on for their children to watch. It's interesting to see how artists do better on one platform and not as well on another.  
...
## Newsworthy pitch and general reportig plan

**A** We've discovered that shorter songs tend to grasp more views, streams and popularity. Why? We think because it can tap into a much deeper study and story findings than simply off of this dataframe.
...
To make this potential story even better, we would first set out to really dig through all of this data from the dataframe to break down length of songs and popularity. We could use graphs using ggplot, we could make graphs using Datawrapper, and also code the correlation between the two. 

Once we do this, we can go out reporting, seeking musical experts and those in the medical field/brain developmental field to discuss short-attention spans and music. We also can get first-hand experiences of people who are music listeners and those who use TikTok. With these quotes, expertise and data, we can craft the most effective story to generational change and where the music industry could be headed. 
